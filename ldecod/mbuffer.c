...
//      error ("max_dec_frame_buffering larger than MaxDpbSize", 500);
	printf("Warning: max_dec_frame_buffering (%d) larger than MaxDpbSize (%d)\n",(int)active_sps->vui_seq_parameters.max_dec_frame_buffering,size);
    }
#endif
  ...
  for(i=0; i<MAXSPS; i++)
  {
    if(curr_subset_sps->Valid && curr_subset_sps->sps.seq_parameter_set_id < MAXSPS)
    {
      ...
      if (curr_subset_sps->sps.vui_parameters_present_flag && curr_subset_sps->sps.vui_seq_parameters.bitstream_restriction_flag)
      {
        ...
        {
          //error ("max_dec_frame_buffering larger than MaxDpbSize", 500);
          printf("Warning: curr_subset_sps->sps.vui_seq_parameters.max_dec_frame_buffering (%d) > curr_subset_sps->sps.max_dec_frame_buffering (%d)\n"
		,(int)curr_subset_sps->sps.vui_seq_parameters.max_dec_frame_buffering,j);
        }
#endif
